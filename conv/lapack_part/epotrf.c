...
/*     Determine the block size for this environment. */
    ...
    nb = ilaenv_(&c__1, "EPOTRF", uplo, n, &c_n1, &c_n1, &c_n1, (ftnlen)6, (
        ftnlen)1);
    if (nb <= 1 || nb >= *n) {

/*        Use unblocked code. */

    epotf2_(uplo, n, &a[a_offset], lda, info);
    } else {
...
/*              Update and factorize the current diagonal block and test
   ...
   Computing MIN */
        ...
        jb = min(i__3,i__4);
        i__3 = j - 1;
        esyrk_("Upper", "Transpose", &jb, &i__3, &c_b13, &a[j *
            a_dim1 + 1], lda, &c_b14, &a[j + j * a_dim1], lda);
        epotf2_("Upper", &jb, &a[j + j * a_dim1], lda, info);
        if (*info != 0) {
            goto L30;
        }
        if (j + jb <= *n) {
...
/*                 Compute the current block row. */
            ...
            i__3 = *n - j - jb + 1;
            i__4 = j - 1;
            egemm_("Transpose", "No transpose", &jb, &i__3, &i__4, &
                c_b13, &a[j * a_dim1 + 1], lda, &a[(j + jb) *
                a_dim1 + 1], lda, &c_b14, &a[j + (j + jb) *
                a_dim1], lda);
            i__3 = *n - j - jb + 1;
            etrsm_("Left", "Upper", "Transpose", "Non-unit", &jb, &
...
/*              Update and factorize the current diagonal block and test
   ...
   Computing MIN */
        ...
        jb = min(i__3,i__4);
        i__3 = j - 1;
        esyrk_("Lower", "No transpose", &jb, &i__3, &c_b13, &a[j +
            a_dim1], lda, &c_b14, &a[j + j * a_dim1], lda);
        epotf2_("Lower", &jb, &a[j + j * a_dim1], lda, info);
        if (*info != 0) {
            goto L30;
        }
        if (j + jb <= *n) {
...
/*                 Compute the current block column. */
            ...
            i__3 = *n - j - jb + 1;
            i__4 = j - 1;
            egemm_("No transpose", "Transpose", &i__3, &jb, &i__4, &
                c_b13, &a[j + jb + a_dim1], lda, &a[j + a_dim1],
                lda, &c_b14, &a[j + jb + j * a_dim1], lda);
            i__3 = *n - j - jb + 1;
            etrsm_("Right", "Lower", "Transpose", "Non-unit", &i__3, &
