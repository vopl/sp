...
/* Subroutine */ int ldpotf2_(char *uplo, integer *n, doublereal *a, integer *
	lda, integer *info)
{
/*  -- LAPACK routine (version 3.1) --   
    ...
    extern doublereal lddot_(const integer *, const doublereal *, const integer *, const doublereal *,
	    const integer *);
    extern /* Subroutine */ int dscal_(const integer *, const doublereal *, const doublereal *, 
	    const integer *);
    extern logical lsame_(const char *, const char *);
    extern /* Subroutine */ int ldgemv_(const char *, const integer *, const integer *,
...
/*           Compute U(J,J) and test for non-positive-definiteness. */
	    ...
	    i__2 = j - 1;
        ajj = a[j + j * a_dim1] - lddot_(&i__2, &a[j * a_dim1 + 1], &c__1,
		    &a[j * a_dim1 + 1], &c__1);
	    if (ajj <= 0.) {
		a[j + j * a_dim1] = ajj;
		goto L30;
...
/*           Compute elements J+1:N of row J. */
		...
		i__2 = j - 1;
		i__3 = *n - j;
        ldgemv_("Transpose", &i__2, &i__3, &c_b10, &a[(j + 1) * a_dim1
			+ 1], lda, &a[j * a_dim1 + 1], &c__1, &c_b12, &a[j + (
			j + 1) * a_dim1], lda);
		i__2 = *n - j;
		d__1 = 1. / ajj;
        ldscal_(&i__2, &d__1, &a[j + (j + 1) * a_dim1], lda);
	    }
/* L10: */
	}
    } else {
...
/*           Compute L(J,J) and test for non-positive-definiteness. */
	    ...
	    i__2 = j - 1;
        ajj = a[j + j * a_dim1] - lddot_(&i__2, &a[j + a_dim1], lda, &a[j
		    + a_dim1], lda);
	    if (ajj <= 0.) {
		a[j + j * a_dim1] = ajj;
		goto L30;
...
/*           Compute elements J+1:N of column J. */
		...
		i__2 = *n - j;
		i__3 = j - 1;
        ldgemv_("No transpose", &i__2, &i__3, &c_b10, &a[j + 1 +
			a_dim1], lda, &a[j + a_dim1], lda, &c_b12, &a[j + 1 + 
			j * a_dim1], &c__1);
		i__2 = *n - j;
		d__1 = 1. / ajj;
        ldscal_(&i__2, &d__1, &a[j + 1 + j * a_dim1], &c__1);
	    }
/* L20: */
	}
    }
