...
/* Subroutine */ int ldpotrf_(char *uplo, integer *n, doublereal *a, integer *
	lda, integer *info)
{
/*  -- LAPACK routine (version 3.1) --   
    ...
    extern /* Subroutine */ int ldgemm_(const char *, const char *, const integer *, const integer *,
    ...
    extern /* Subroutine */ int ldtrsm_(const char *, const char *, const char *, const char *,
    ...
    extern /* Subroutine */ int ldsyrk_(const char *, const char *, const integer *, const integer *,
	    const doublereal *, const doublereal *, const integer *, const doublereal *, const doublereal *, 
         const integer *), ldpotf2_(const char *, const integer *,
...
/*        Use unblocked code. */
    ...
    ldpotf2_(uplo, n, &a[a_offset], lda, info);
    } else {
...
/*              Update and factorize the current diagonal block and test   
   ...
   Computing MIN */
		...
		jb = min(i__3,i__4);
		i__3 = j - 1;
        ldsyrk_("Upper", "Transpose", &jb, &i__3, &c_b13, &a[j *
			a_dim1 + 1], lda, &c_b14, &a[j + j * a_dim1], lda);
        ldpotf2_("Upper", &jb, &a[j + j * a_dim1], lda, info);
		if (*info != 0) {
		    goto L30;
		}
		if (j + jb <= *n) {
...
/*                 Compute the current block row. */
		    ...
		    i__3 = *n - j - jb + 1;
		    i__4 = j - 1;
            ldgemm_("Transpose", "No transpose", &jb, &i__3, &i__4, &
			    c_b13, &a[j * a_dim1 + 1], lda, &a[(j + jb) * 
			    a_dim1 + 1], lda, &c_b14, &a[j + (j + jb) * 
			    a_dim1], lda);
		    i__3 = *n - j - jb + 1;
            ldtrsm_("Left", "Upper", "Transpose", "Non-unit", &jb, &
...
/*              Update and factorize the current diagonal block and test   
   ...
   Computing MIN */
		...
		jb = min(i__3,i__4);
		i__3 = j - 1;
        ldsyrk_("Lower", "No transpose", &jb, &i__3, &c_b13, &a[j +
			a_dim1], lda, &c_b14, &a[j + j * a_dim1], lda);
        ldpotf2_("Lower", &jb, &a[j + j * a_dim1], lda, info);
		if (*info != 0) {
		    goto L30;
		}
		if (j + jb <= *n) {
...
/*                 Compute the current block column. */
		    ...
		    i__3 = *n - j - jb + 1;
		    i__4 = j - 1;
            ldgemm_("No transpose", "Transpose", &i__3, &jb, &i__4, &
			    c_b13, &a[j + jb + a_dim1], lda, &a[j + a_dim1], 
			    lda, &c_b14, &a[j + jb + j * a_dim1], lda);
		    i__3 = *n - j - jb + 1;
            ldtrsm_("Right", "Lower", "Transpose", "Non-unit", &i__3, &
