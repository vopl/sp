...
#define BOXPROJECTCHECK LM_ADD_PREFIX(boxProjectCheck)
...
static int BOXPROJECTCHECK(LM_REAL *p, LM_REAL *lb, LM_REAL *ub, int m)
{
	register int i;
   ...
   /* compute initial damping factor */
    ...
    while(1){
      ...
      if(issolved){
		...
		if(!BOXPROJECTCHECK(pDp, lb, ub, m))
		{
			stop=91;
			break;
		}
