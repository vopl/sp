...
Clarity_Convolve( double* inImage, Clarity_Dim3 imageDim,
  ...
  double *kernelPad = (double *) malloc(sizeof(double)*workVoxels);
  ...
  double *outImagePad = (double *) malloc(sizeof(double)*workVoxels);
   ...
   if (g_CUDACapable) {
      ...
      result = Clarity_Real_MallocCopy((void **) &psf, 
         sizeof(double), workDim.x, workDim.y, workDim.z, kernelPad);
      if (result != CLARITY_SUCCESS) {
         Clarity_Free(in);
         return result;
      }
...
Clarity_Convolve_OTF(
   int nx, int ny, int nz, double* in, double* otf, double* out) {
   ...
   result = Clarity_FFT_C2R_double(nx, ny, nz, inFT, out);
   Clarity_Free(inFT);
...
Clarity_ConvolveInternal(
   int nx, int ny, int nz, double* in, double* psf, double* out) {
...
Clarity_Modulate_KernelGPU(
   int nx, int ny, int nz, double* inFT, double* otf, double* outFT);
#endif
