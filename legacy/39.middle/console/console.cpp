...
#define TIMEZERO "0001-00-01 00:00:00"
#define TIMEBEG "2004-05-17 00:00:00"
				...
				std::ofstream &outResponse)
{
	...
	//отклик
	{
		...
		for(size_t i(0); i<s; i++)
		{
			...
			outResponse<<value[i].a()<<"\t";
			outResponse<<value[i].p()<<"\n";
		}
		outResponse.flush();
	}
...
int processOne(int pow)
{
	...
	if(!core->load(stateFileName.c_str()))
	{
		...
		ssd._zeroPeriod = (60);//60 сек
		ssd._smoothLength = 10;	//N по ssd._zeroPeriod
...
// 	}
	if(g_data.empty())
	{
		loadData("P:\\projects\\finance\\quotes\\crosses\\ocs\\EURXAG", g_data);
		if(g_data.empty())
		{
	...
	for(size_t idx=1; idx<g_data.size(); idx++)
	{
		...
		{
			core->update(TIMESTEP, true, false);
