...
Clarity_MaximumLikelihoodUpdate(
   int nx, int ny, int nz, double* in, double energy,
   double* currentGuess, double* otf, double* s1, double* s2, 
   double* newGuess) {
...
Clarity_MaximumLikelihoodDeconvolveCPU(
   ...
   double* s1 = NULL;
   result = Clarity_Real_Malloc((void**) &s1, sizeof(double),
      nx, ny, nz);
   if (result != CLARITY_SUCCESS) {
   ...
   double* s2 = NULL;
   result = Clarity_Real_Malloc((void**) &s2, sizeof(double),
      nx, ny, nz);
   if (result != CLARITY_SUCCESS) {
   ...
   for (unsigned k = 0; k < iterations; k++) {
	   double* currentGuess = (k == 0 ? inImage : iPtr);
      double* newGuess     = (k == iterations-1 ? outImage : iPtr);
...
Clarity_MaximumLikelihoodDeconvolveGPU(
   ...
   // Iterate
   for (unsigned k = 0; k < iterations; k++) {
      double* currentGuess = (k == 0 ? in : iPtr);
      double* newGuess     = iPtr;
