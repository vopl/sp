...
__global__ void ModulateCUDAKernel(int n, double scale, Complex* inFT, Complex* psfFT, Complex* outFT) {
...
extern "C"
void
Clarity_Modulate_KernelGPU(int nx, int ny, int nz, double* inFT,
                           double* psfFT, double* outFT) {
   int n = nz*ny*(nx/2 + 1);
   dim3 grid(BLOCKS);
   dim3 block(THREADS_PER_BLOCK);
   double scale = 1.0f / ((double) nx*ny*nz);
