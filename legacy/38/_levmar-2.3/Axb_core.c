...
int AX_EQ_B_LU(LM_REAL *A, LM_REAL *B, LM_REAL *x, int m)
{
#define STATICBUFLEN 32768
	static staticBuf[STATICBUFLEN];
...
#ifdef LINSOLVERS_RETAIN_MEMORY
  if(tot_sz>buf_sz){ /* insufficient memory, allocate a "big" memory chunk at once */
    if(buf && buf != staticBuf) free(buf); /* free previously allocated memory */

	if(tot_sz <= STATICBUFLEN)
	{
		buf_sz = tot_sz;
		buf = staticBuf;
	}
