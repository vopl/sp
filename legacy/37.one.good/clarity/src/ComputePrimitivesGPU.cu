...
void
ReduceSumKernelGPU(
   double* blockResults, double* data, int n, int padN) {
...
Clarity_ReduceSumGPU(
   double* result, double* buffer, int n) {
   
   // Set up device call configuration.
   dim3 blockSize(THREADS_PER_BLOCK);
   dim3 gridSize(BLOCKS);
   size_t sharedSize = sizeof(double)*blockSize.x;
   ...
   double *blockResultsDev = NULL;
   cudaMalloc((void**)&blockResultsDev, sizeof(double)*gridSize.x);
   ...
   double* blockResultsHost = (double*) malloc(sizeof(double)*gridSize.x);
   cudaMemcpy(blockResultsHost, blockResultsDev, 
      sizeof(double)*gridSize.x, cudaMemcpyDeviceToHost);
...
DivideArraysComponentWiseKernelGPU(
   double* result, double* a, double* b, double value, int n) {
...
void
ScaleArrayKernelGPU(
   double* result, double* a, int n, double scale) {
