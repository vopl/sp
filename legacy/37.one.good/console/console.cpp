...
#define TIMEBEG "2008-07-02 06:00:00"
#define TIMEEND "2009-10-31 00:00:00"

#define TIMESTEP (60*10)
#define SPECTR_SAVE_CNT 10
...
//		real x0 = core->getLastDataTime();
		...
		for(size_t i(0); i<s; i++)
		{
			outSpectr<<x0<<"\t";
			outSpectr<<period[i]<<"\t";
			outSpectr<<value[i].re()<<"\t";
			outSpectr<<value[i].im()<<"\n";
		}
		outSpectr.flush();
	}
...
// 	}
	...
	for(size_t idx=1; idx<g_data.size(); idx++)
	{
		...
		while(core->needUpdate())
		{
			...
			real error = core->getSeparatorError(0);
