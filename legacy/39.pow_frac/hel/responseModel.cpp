...
// 	}
	...
	void ResponseModel::evalFuncOpt(double *p, double *hx, int m, int n, void *_SFuncOptParams)
	{
		...
		if(params->_slogt)
		{//значени€ периода спектра. ≈сли задано то период спектра Ќ≈ оптимизируетс€
			int mDiv2 = m/2;
			for(int i(0); i<nDiv2; i++)
			{
				...
				for(int j(0); j<mDiv2; j++)
				{
					...
					//assert(!"вычислени€ переделать на таблицу");
		...
		{//значени€ периода спектра. ≈сли Ќ≈ задано то период спектра оптимизируетс€
			int mDiv3 = m/3;
			for(int i(0); i<nDiv2; i++)
			{
				logt = params->_elogt[i];
		...
		//значени€ отклика, если задано, то будет вычтено из вычесленного (можно не задавать в dlrvmar_der)
		if(params->_ev)
		{
			for(int i(0); i<nDiv2; i++)
			{
...
// 		exit(0);
		...
		TVReal &work)
	{
		...
		assert(sizeof(double) == sizeof(Complex)/2);//хак Complex[n] используетс€ как double[n*2]
